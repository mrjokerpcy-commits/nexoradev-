...
async function setup() {
  ...
  try {
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_chat_recipient ON chat_messages(recipient_id);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_chat_sender ON chat_messages(sender_id);`);

    // ── LEADS TABLE (from portfolio contact form) ──
    await client.query(`
      CREATE TABLE IF NOT EXISTS leads (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        ticket_id VARCHAR(20) UNIQUE NOT NULL DEFAULT 'NX-' || UPPER(SUBSTRING(gen_random_uuid()::text, 1, 8)),
        ticket_code VARCHAR(10) NOT NULL DEFAULT UPPER(SUBSTRING(MD5(RANDOM()::text), 1, 6)),
        name VARCHAR(100) NOT NULL,
        email VARCHAR(255) NOT NULL,
        service VARCHAR(100),
        budget VARCHAR(50),
        message TEXT NOT NULL,
        status VARCHAR(20) DEFAULT 'open' CHECK (status IN ('open','replied','converted','closed')),
        created_at TIMESTAMPTZ DEFAULT NOW()
      );
    `);
